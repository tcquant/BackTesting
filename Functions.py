...
def trades(futures, gen_call, gen_put, io=0,gap = 15,delta_range=0.1,DeltaUB = 0.5):
    Securities = []
    entry = False
        ...
        for i in range(1, len(futures) - 1):
            ...
            for k, v in positions.items():
                ...
                if dt <= k_datetime + pd.to_timedelta(15, unit='m') and v['null'] == False and not v['entry']:
                    ...
                    print(f"Checking For subcribed Securities for Stamp for taking entry {k}",file=file)
            ...
            for k, v in positions.items():
                if not v['null'] and v['entry'] and not v['exit']:
                    if dt >= v['ent']['dt'] + pd.to_timedelta(15, unit='m'):
                        print(f"Checking for Squreoff condition for {k}",file=file)
